...
class EvenementController extends Controller
{
    ...
    public function listAction(Request $request){
        ...
        $date = date("m/d/y");
        $em = $this->getDoctrine()->getManager();
        $evnt = $em->getRepository("UserBundle:Evenement")->findBy([], ['date' => 'DESC']);
        ...
        foreach($evnt as $test ){
            $ide = $test->getIdEvent();
            $rateevnt = $em->getRepository("UserBundle:Rating")->getEvenementsRatings($ide)[0]['val'];
        ...
        return $this->render("@User/Evenement/list_evenement.html.twig",array(
            'events'=>$result ,'msgs'=>$msg,'rates'=>$rate,'date'=>$date
        ));
    }
    ...
    public function AnnulerParticipationAction(Request $request, $id)
    {
       ...
       // $em=$this->getDoctrine()->getManager();
        $Demande= new Demande();
      //  $Demande=$em->getRepository("UserBundle:Demande")->findOneByidevent($id);
      //  $Demande->setEtat("non interssé");
       // $em->persist($Demande);
      //  $em->flush();
      //  $etat="interssé";
